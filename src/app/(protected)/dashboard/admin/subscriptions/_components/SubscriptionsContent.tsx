...
export function SubscriptionsContent({ initialPayments, initialMeta, accessToken }: SubscriptionsContentProps) {
  ...
  function fetchPayments(p: number) {
    setLoading(true)
    adminGetSubscriptions(accessToken, { page: String(p), limit: "10" }).then((res) => {
      if (res.success) {
  ...
  async function handleStatusUpdate(paymentId: string, status: string) {
    ...
    const res = await adminUpdatePaymentStatus(accessToken, paymentId, status)
    if (res.success) {
