...
import { Search, Trash2, AlertTriangle, ExternalLink, Car } from "lucide-react"
...
export default function AdminRidesPage() {
  ...
  const fetchRides = useCallback((p: number, s: string) => {
    ...
    adminGetRides(session.user.accessToken, query).then((res) => {
      if (res.success) {
        setRides(res.data)
        setMeta(res.meta)
      }
  ...
  async function handleDelete(rideId: string) {
    ...
    const res = await adminSoftDeleteRide(session.user.accessToken, rideId)
    if (res.success) {
