...
export function DriverDashboardContent({ initialData, initialSub }: Props) {
  const { data: session } = useSession()
  const router = useRouter()
  const [data, setData] = useState<DriverData>(initialData)
  const [subscribed, setSubscribed] = useState(initialSub.isSubscribed)
  const [subExpiry, setSubExpiry] = useState<string | null>(initialSub.expiryDate)
  ...
  const todayRidesList = (data.recentRides || []).filter((r) => {
    const d = new Date(r.createdAt)
    d.setHours(0, 0, 0, 0)
    return d.getTime() === today.getTime() && r.status === "COMPLETED"
  })
  const todayEarnings = todayRidesList.reduce((sum, r) => sum + (r.systemSuggestedFare || 0), 0)
  const todayRides = todayRidesList.length

  const fetchData = useCallback(() => {
    if (!accessToken) return
    getDriverDashboard(accessToken).then((res) => {
  ...
  async function handleStartRide(rideId: string) {
    ...
    const res = await startRide(rideId, accessToken)
    if (res.success) {
  ...
  async function handleCompleteRide(rideId: string) {
    if (!accessToken) return
    setActionLoading(rideId)
    const res = await completeRide(rideId, accessToken)
    if (res.success) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatsCard
          ...
          value={data.totalRidesCompleted}
          ...
          <CardContent>
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between py-2">
                ...
                <span className="text-lg font-bold text-foreground">{data.totalRidesCompleted}</span>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      {session?.user.id && accessToken && (
        <RecentReviewsCard driverId={session.user.id} accessToken={accessToken} />
      )}
