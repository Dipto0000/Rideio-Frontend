...
interface Props {
  initialData: RiderData
}

export function RiderDashboardContent({ initialData }: Props) {
  const { data: session } = useSession()
  const router = useRouter()
  const [data, setData] = useState<RiderData>(initialData)
  ...
  const fetchData = useCallback(() => {
    ...
    import("@/lib/actions/dashboard.actions").then(({ getRiderDashboard }) => {
      getRiderDashboard(accessToken).then((res) => {
  ...
  async function handleCancelRide(rideId: string) {
    ...
    const res = await cancelRide(rideId, accessToken)
    if (res.success) {
                    ...
                    >
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          {ride.status === "COMPLETED" && accessToken && (
                            <ReviewDialog
                              rideId={ride._id}
